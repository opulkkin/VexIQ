...
#pragma config(Motor,  motor1,          leftMotor1,    tmotorVexIQ, openLoop, driveLeft, encoder)
#pragma config(Motor,  motor6,          rightMotor1,   tmotorVexIQ, openLoop, reversed, driveRight, encoder)
#pragma config(Motor,  motor7,          leftMotor2,    tmotorVexIQ, PIDControl, driveLeft, encoder)
#pragma config(Motor,  motor12,         rightMotor2,   tmotorVexIQ, PIDControl, reversed, driveRight, encoder)
...
task main()
{
  ...
  setMotorBrakeMode(leftMotor1, motorCoast);     // Robottia on helpompi siirreella lykkaamalla
  setMotorBrakeMode(rightMotor1, motorCoast);    // talla asetuksella
  setMotorBrakeMode(leftMotor2, motorCoast);     // Robottia on helpompi siirreella lykkaamalla
  setMotorBrakeMode(rightMotor2, motorCoast);    // talla asetuksella
  ...
  while(true)     // Ikuinen silmukka
	{
	  ...
	  if(abs(kanavaEteenTaakseLukema) > PYSAYTYSRAJA || abs(kanavaOikealleVasemmalleLukema) > PYSAYTYSRAJA){
	    ...
	    asetaMoottorienNopeudet(kanavaEteenTaakseLukema, kanavaOikealleVasemmalleLukema);
			...
			setMotorSpeed(leftMotor1, 0);    // Jos kumpikaan getJoystickValue arvoista ei ole itseisarvoltaan pysaytysrajaa suurempi
																			// niin pysaytetaan.
			setMotorSpeed(rightMotor1, 0);

			setMotorSpeed(leftMotor2, 0);    // Jos kumpikaan getJoystickValue arvoista ei ole itseisarvoltaan pysaytysrajaa suurempi
																			// niin pysaytetaan.
...
void asetaMoottorienNopeudet(short channelEteenTaakse, short channelOikealleVasemmalle){
  ...
  setMotorSpeed(leftMotor1,  vasenNopeus);

	setMotorSpeed(rightMotor1, oikeaNopeus);

	setMotorSpeed(leftMotor2,  vasenNopeus);

	setMotorSpeed(rightMotor2, oikeaNopeus);
